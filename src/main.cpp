...
int main() {
    ...
    scene.addSphere(2.0, vec4(7.5, -2.5, -0.5, 1.0), MAGENTA, SurfaceType::LAMBERTIAN);
    scene.addSphere(2.0, vec4(7.5,  2.5, -0.5, 1.0), WHITE, SurfaceType::LAMBERTIAN);
    // Add point lights for local lighting
    scene.addPointLight({ glm::vec4(5.0, 0.0, 4.5, 1.0), 0.2, WHITE });
    scene.addPointLight({ glm::vec4(2.0, 4.0,-4.0, 1.0), 0.8, WHITE });
    scene.addPointLight({ glm::vec4(2.0,-4.0,-4.0, 1.0), 0.5, WHITE });
    ...
    // Create the camera
    Camera camera = Camera{ glm::vec4(-2.0, 0.0, 0.0, 1.0), glm::vec4(-1.0, 0.0, 0.0, 1.0), 4 };
